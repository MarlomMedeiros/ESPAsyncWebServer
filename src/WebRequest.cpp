...
const char* AsyncWebServerRequest::methodToString() const {
  if (_method == AHTTP_ANY)
    return T_ANY;
  if (_method & AHTTP_GET)
    return T_GET;
  if (_method & AHTTP_POST)
    return T_POST;
  if (_method & AHTTP_DELETE)
    return T_DELETE;
  if (_method & AHTTP_PUT)
    return T_PUT;
  if (_method & AHTTP_PATCH)
    return T_PATCH;
  if (_method & AHTTP_HEAD)
    return T_HEAD;
  if (_method & AHTTP_OPTIONS)
    return T_OPTIONS;
  return T_UNKNOWN;
}
