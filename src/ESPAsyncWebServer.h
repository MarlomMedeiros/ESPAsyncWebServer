...
class AsyncWebServerRequest {
  ...
  public:
    ...
    bool isWebSocketUpgrade() const { return _method == AHTTP_GET && isExpectedRequestedConnType(RCT_WS); }
    bool isSSE() const { return _method == AHTTP_GET && isExpectedRequestedConnType(RCT_EVENT); }
...
class AsyncWebServer : public AsyncMiddlewareChain {
  ...
  public:
    ...
    AsyncCallbackWebHandler& on(const char* uri, ArRequestHandlerFunction onRequest) { return on(uri, AHTTP_ANY, onRequest); }
