...
request->method();        // enum:    AHTTP_GET, AHTTP_POST, AHTTP_DELETE, AHTTP_PUT, AHTTP_PATCH, AHTTP_HEAD, AHTTP_OPTIONS
...
server.on("/scan", AHTTP_GET, [](AsyncWebServerRequest *request){
  String json = "[";
...
void setup(){
  ...
  server.on("/heap", AHTTP_GET, [](AsyncWebServerRequest *request){
    request->send(200, "text/plain", String(ESP.getFreeHeap()));
  });

  // upload a file to /upload
  server.on("/upload", AHTTP_POST, [](AsyncWebServerRequest *request){
    request->send(200);
  }, onUpload);

  // send a file when /index is requested
  server.on("/index", AHTTP_ANY, [](AsyncWebServerRequest *request){
    request->send(SPIFFS, "/index.htm");
  });

  // HTTP basic authentication
  server.on("/login", AHTTP_GET, [](AsyncWebServerRequest *request){
  ...
  server.on("/update", AHTTP_GET, [](AsyncWebServerRequest *request){
    request->send(200, "text/html", "<form method='POST' action='/update' enctype='multipart/form-data'><input type='file' name='update'><input type='submit' value='Update'></form>");
  });
  server.on("/update", AHTTP_POST, [](AsyncWebServerRequest *request){
...
public :
  ...
  void begin(){
    ...
    classWebServer.on("/example", AHTTP_ANY, handleRequest);

    // attach class request handler
    classWebServer.on("/example", AHTTP_ANY, std::bind(&WebClass::classRequest, this, std::placeholders::_1));
  }
};
...
void setup() {
  // attach global request handler
  globalWebServer.on("/example", AHTTP_ANY, handleRequest);

  // attach class request handler
  globalWebServer.on("/example", AHTTP_ANY, std::bind(&WebClass::classRequest, webClassInstance, std::placeholders::_1));
}
...
webServer.onNotFound([](AsyncWebServerRequest *request) {
  if (request->method() == AHTTP_OPTIONS) {
    request->send(200);
  } else {
    request->send(404);
  }
...
```cpp
  server.on("^\\/sensor\\/([0-9]+)$", AHTTP_GET, [] (AsyncWebServerRequest *request) {
